...
<<<<<<< HEAD
    ...
    for muni in municipalities.values():
        if not muni.weather_history:
            continue
        # current pipeline appends exactly one record per muni; still handle list safely
        for record in muni.weather_history:
            t = record.get("avg_temp_yearly")
            s = record.get("sunny_days_count")
            r = record.get("rainy_days_count")
            f = record.get("foggy_days_count")
            q = record.get("avg_aqi")
            if t is not None: temps.append(float(t))
            if s is not None: sunny.append(float(s))
            if r is not None: rainy.append(float(r))
            if f is not None: foggy.append(float(f))
            if q is not None: aqi.append(float(q))

    if not (temps and sunny and rainy and foggy and aqi):
        logger.warning("No weather data available for scoring (expected avg_temp_yearly/sunny_days_count/rainy_days_count/foggy_days_count/avg_aqi)")
        return

    tmin, tmax = min(temps), max(temps)
    smin, smax = min(sunny), max(sunny)
    rmin, rmax = min(rainy), max(rainy)
    fmin, fmax = min(foggy), max(foggy)
    qmin, qmax = min(aqi), max(aqi)
    ...
    def norm_lo(x, lo, hi):
        # lower is better
        return 1.0 - ((x - lo) / (hi - lo)) if hi > lo else 0.5

    for muni in municipalities.values():
        if not muni.weather_history:
            continue

        # Use first record (most municipalities have just one yearly record)
        record = muni.weather_history[0]
        ...
        if None in (t, s, r, f, q):
            continue

        t = float(t); s = float(s); r = float(r); f = float(f); q = float(q)

        # Weighted score in 0..100
        score01 = (
            0.25 * norm_hi(t, tmin, tmax) +
            0.25 * norm_hi(s, smin, smax) +
            0.20 * norm_lo(q, qmin, qmax) +
            0.20 * norm_lo(r, rmin, rmax) +
